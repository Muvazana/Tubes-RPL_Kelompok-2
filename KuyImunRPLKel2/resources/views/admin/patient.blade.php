                                    ...
                                    @foreach ($data as $user) 
                                        <tr class="bg-white border-collapse">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                {{ $user->user_members->status == "verified" ? "Verified" : "Not Verified" }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3 w-10">
                                                <a href="{{ url('/admin/patient/edit/'.$user->id) }}" class="px-5 py-2 bg-blue-500 rounded-md text-white">Edit</a>
                                                <a href="{{ url('/admin/patient/delete/'.$user->id) }}" class="px-5 py-2 bg-red-500 rounded-md text-white">Delete</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
